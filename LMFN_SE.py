...
class LMFN(nn.Module):
    ...
    def __init__(self, in_channel, classes, kernel_nums=1, spe_kernel_depth=7, spa_kernel_size=5, init_conv_stride=1,
        ...
        self.se = SE_Block(channel=spa_kernel_depth)
        self.apply(self.weight_init)
    ...
    def forward(self, x):
        ...
        x5 = self.se(torch.squeeze(x0)) + x5 # frist attention

        x6 = self.spatial_conv2(x5)

        x6 = self.se(torch.squeeze(x2)) + x6 # second attention

        x7 = self.spatial_conv3(x6)

        x7 = self.se(x4) + x7 # thrid attention
