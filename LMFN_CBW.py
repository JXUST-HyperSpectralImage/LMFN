...
class CBW_BasicBlock(nn.Module):
    ...
    def __init__(self, in_channels, stride=1, padding=1, patch_size=11, count=1):
...
class SLFN(nn.Module):
    ...
    def __init__(self, in_channel, classes, kernel_nums=1, spe_kernel_depth=7, spa_kernel_size=5, init_conv_stride=1,
        ...
        self.cbw = CBW_BasicBlock(spa_kernel_depth, stride=1, padding=1, patch_size=9, count=1)
    ...
    def forward(self, x):
        ...
        x5 = self.cbw(torch.squeeze(x0)) + x5 # frist attention
        x6 = self.spatial_conv2(x5)
        x6 = self.cbw(torch.squeeze(x2)) + x6 # second attention
        x7 = self.spatial_conv3(x6)
        x7 = self.cbw(x4) + x7 # thrid attention
